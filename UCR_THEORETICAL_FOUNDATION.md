...
intervals.icuから連携される、または手動で記録される主観的指標（疲労度、ストレス、モチベーション、気分、1-5段階）は、まず方向性を「高いほど良い」に統一します。その後、**重み付き平均**を用いて0から20点の連続的な「主観的ウェルネススコア」に変換します。
...
$$Score\_Wellness = 20 \times \sum_{i} \left( \frac{5 - \text{Value}_i}{4} \times \text{Weight}_i \right) \times (1 - \text{MissingPenalty})$$

ここで、$\text{Value}_i$は各指標のintervals.icu値（1=最良、5=最悪）、$\text{Weight}_i$は各指標の重み、$\text{MissingPenalty}$は欠損データペナルティ（欠損項目数×5%）です。

...
2. **バッファーゾーンを考慮した線形関数によるマッピング**: HRVと同様に、正常範囲内での過度なペナルティを避けるため、ベースラインと傾きを調整します：
   ...
   $$Score\_RHR = \max(0, \min(20, 14 + (z_{RHR} \cdot 6)))$$
   
   この調整により：
   - **ベースライン = 14点**：平均的なRHR（Z=0）の状態を70%（14/20）と評価し、「準備OK」の位置づけにします
   - **傾き = 6**：RHRの僅かな変動に対するスコアの変化を緩やかにし、日常的な変動に対してモデルをより安定（ロバスト）にします
   
   例えば、RHRがわずか0.25標準偏差悪化（z_RHR = -0.25）した場合でも、スコアは12.5点（62.5%）となり、依然として合理的な範囲内に留まります。

### **2.4. 睡眠スコア：高解像度情報の維持**

Garminの睡眠スコアは既に0から100の連続値として提供されるため、これをカテゴリに分類する（ビニング）ことは不必要な情報損失です。単純な**線形スケーリング**によって、0から20点のコンポーネントスコアに変換します。

$$Score\_Sleep = \frac{\text{GarminSleepScore}}{100} \cdot 20$$

これにより、睡眠の質のわずかな違いも最終的なレディネススコアに滑らかに反映されます。
